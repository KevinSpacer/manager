...
// 全局模块
const { eatFoodModule } = proxy.$usePiniaModule();
// console.log(eatFoodModule.paymentDetails);
const {
  toolForm = {},
  goodsDetailed = {},
  getTotalOrderAmount = {},
...
// 发起AA付款申请
const initiateOrderAaPay = async () => {
  const id = routeParams.orderId;
  const peopleQuantity = Number(routeParams.peopleQuantity);
  const val = getTotalOrderAmount.payPrice / peopleQuantity;
  const amount = new Array(peopleQuantity).fill(val);
  const params = {
    amount,
    dividedNumber: peopleQuantity,
    id,
  };
  try {
    await proxy.$storeDispatch("fetchInitiateOrderSharePay", params);
  } catch (error) {
    console.log(error);
  }
};
