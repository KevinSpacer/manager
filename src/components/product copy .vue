      ...
      <div class="productInfo">
        ...
        <div class="bottom">
          ...
          <div class="tool">
            <!-- 规格按钮 -->
            <div
              class="change-spec oneLineOver"
              v-if="showSpecBtn"
              @click.stop="changeSpec"
            >
              {{ proxy.$LANG_TEXT("选规格") }}
            </div>
            <!-- 数量选择 -->
            <div class="change-spec" v-else @click.stop="pushNewGoods">
              <el-icon bold><Plus /></el-icon>
              <span>{{ proxy.$LANG_TEXT("选择") }}</span>
            </div>
            <!-- <div class="change-num" v-else>
							<number-change
								@changeCount="changeCount"
								:addDisabled="isOutArea"
								v-model="carNum"
							></number-change>
						</div> -->
          </div>
